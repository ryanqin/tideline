...
# --- native_gloss column retirement --------------------------------------


def test_native_gloss_column_dropped_on_migration():
    """A pre-existing translations table carrying the deprecated native_gloss
    column is upgraded in place: init drops it (target == native now, so a
    gloss into the first language is moot). Existing rows survive."""
    from tideline.tools.translation import init_db

    conn = sqlite3.connect(":memory:")
    conn.execute(
        "CREATE TABLE translations (id INTEGER PRIMARY KEY AUTOINCREMENT, "
        "original TEXT NOT NULL, target_lang TEXT NOT NULL, translated TEXT NOT NULL, "
        "source_lang TEXT, native_gloss TEXT, source TEXT, context_snippet TEXT, "
        "session_id TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)"
    )
    conn.execute(
        "INSERT INTO translations (original, target_lang, translated, native_gloss) "
        "VALUES ('ラーメン', 'Chinese', '拉面', '拉面')"
    )
    conn.commit()
    assert "native_gloss" in {r[1] for r in conn.execute("PRAGMA table_info(translations)")}

    init_db(conn)   # triggers the DROP COLUMN migration

    cols = {r[1] for r in conn.execute("PRAGMA table_info(translations)")}
    assert "native_gloss" not in cols
    assert conn.execute("SELECT original FROM translations").fetchone()[0] == "ラーメン"
    conn.close()
