...
| 2026-06-09 | **Source image kept as recall material** (storage foundation) — a capture's photo is no longer discarded once the VLM has read it into a scene gist: `translations.source_image` (BLOB) stores it, the capture client injects the bytes via context (never the LLM, which stays text-only), and `GET /api/translations/{id}/image` serves it back. Seed gives each image session a placeholder swatch. Display / review use of the photo is a later slice | §3.2; `tools/translation.py`, `seed.py`, `web/app.py` |
